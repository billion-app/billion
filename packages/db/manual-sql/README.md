...
One-off **data** operations that were applied by hand against the database.
These are historical records, not part of the Drizzle migration pipeline — they
mutate rows rather than schema, so `drizzle-kit generate` cannot represent them.
