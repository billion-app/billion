...
export function generatedEnvFromTemplate(template) {
  const firstSectionEnd = template.indexOf("\n\n");
  const body =
    firstSectionEnd >= 0 ? template.slice(firstSectionEnd + 2) : template;
  return [
    "# THIS FILE WAS GENERATED BY `pnpm onboard`.",
    "# It may contain secrets. Do not commit it.",
    "# Use `pnpm env:setup` to add or update configuration.",
    "",
    body,
  ].join("\n");
}
  ...
  if (!existsSync(ENV_PATH)) {
    if (await prompt.confirm("Create .env from .env.example?")) {
      if (options.dryRun) log("  [dry run] Copy .env.example to .env");
        ...
        writeFileSync(ENV_PATH, generatedEnvFromTemplate(template), "utf8");
      }
