...
export function SponsorCard({
  ...
  return (
    <div
      className="bg-slate border-hair-1 flex items-center gap-3 rounded-[14px] border p-3"
      data-testid="bill-sponsor-card"
    >
      {sponsor.imageUrl ? (
        <img
          src={sponsor.imageUrl}
          alt=""
          className="bg-surface h-11 w-11 rounded-full object-cover"
        />
      ) : (
      ...
      <div className="min-w-0">
        ...
        <p className="truncate font-sans text-[15px] font-bold">
          {sponsor.name}
        </p>
        {meta ? (
          <p className="text-quiet truncate font-sans text-[12.5px]">{meta}</p>
        ) : null}
      </div>
    </div>
  );
