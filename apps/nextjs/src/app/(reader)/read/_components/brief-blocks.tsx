...
const DIRECTION: Record<string, { icon: IconName; label: string; color: string }> = {
...
export function BriefBlocks({
  ...
  return (
    <div className="flex flex-col gap-6" data-testid="bill-brief">
      ...
      {brief.whyNotBefore ? <WhyNotBefore context={brief.whyNotBefore} accent={accent} /> : null}
      ...
      {brief.deepDive || reading.length > 0 ? (
        <>
          ...
          <FurtherReading deepDive={brief.deepDive} reading={reading} accent={accent} />
        </>
      ) : null}
    </div>
  );
...
function Chevron() {
  return (
    <Icon name="chevD" size={14} className="shrink-0 transition-transform group-open:rotate-180" />
  );
}
  ...
  return (
    ...
    >
      <div className="flex items-center gap-[9px]">
        ...
        <h2 className="font-editorial flex-1 text-[17px] font-bold text-white">The short version</h2>
        <span
      ...
      {hasExtended ? (
        <details className="group">
          ...
          >
            ...
            <span className="hidden group-open:inline">Hide extended version</span>
            <Chevron />
          </summary>
          <div className="border-hair-1 mt-3 flex flex-col gap-[7px] border-t pt-[13px]" data-testid="brief-hook">
            <p className="text-quiet font-sans text-[9.5px] font-semibold tracking-[0.1em]">EXTENDED VERSION</p>
...
function WhyNotBefore({
  ...
  const number = new Map(sources.map((source, index) => [source.url, index + 1]));
  return (
    <details className="group bg-slate border-hair-1 rounded-[14px] border p-4" data-testid="brief-why-not-before">
      ...
      <div className="mt-4 flex flex-col gap-4">
        ...
        <ol className="m-0 flex list-none flex-col gap-3 p-0">
          {context.points.map((point, index) => (
            <li key={index} className="flex gap-3">
              <span className="shrink-0 font-sans text-[13px] font-bold tabular-nums" style={{ color: accent }}>
        ...
        <div className="border-hair-1 border-t pt-3">
          <p className="text-quiet mb-2 font-sans text-[11px] font-bold tracking-[0.1em] uppercase">Sources</p>
          <ol className="m-0 flex list-none flex-col gap-2 p-0">
            {sources.map((source, index) => (
              <li key={source.url}>
                ...
                >
                  <span className="font-sans text-[12px] font-bold" style={{ color: accent }}>
                    [{index + 1}]
                  </span>
                  <span className="flex-1">
                    ...
                    <span className="text-quiet block font-sans text-[12px]">{source.publisher}</span>
                  </span>
...
/* ---------- key terms ---------- */
export function Terms({ terms, accent }: { terms: Brief["terms"]; accent: string }) {
    ...
    <section data-testid="brief-key-terms">
      <div className="mb-3 flex items-center gap-[10px]">
        ...
        <div>
          ...
          <p className="text-quiet font-sans text-[12px]">Know these before you read further</p>
        </div>
      </div>
      <dl className="m-0 flex flex-col gap-3">
        {terms.map((term, index) => {
          ...
          return (
            ...
            >
              <dt className="font-sans text-[13.5px] font-bold" style={{ color }}>
                {term.term}
              </dt>
  ...
  return (
    ...
    <div className="grid gap-3 md:grid-cols-2" data-testid="brief-changes">
      {changes.map((change, index) => (
        <article key={index} className="bg-slate border-hair-1 flex flex-col rounded-[16px] border p-4">
          ...
          <h3 className="mb-3 font-sans text-[16.5px] leading-[22px] font-semibold">{change.title}</h3>
          ...
          <div className="rounded-[12px] bg-[rgba(255,255,255,0.03)] p-3">
            <p className="text-quiet mb-1 font-sans text-[9.5px] font-bold tracking-[0.12em]">NOW</p>
            <p className="font-sans text-[14px] leading-[20px] text-white/[0.72]">
              <Emphasis text={change.before} />
            </p>
          </div>
          <div className="flex items-center gap-2 py-2 pl-3" style={{ color: accent }}>
            <Icon name="arrowDown" size={13} />
            <span className="font-sans text-[9.5px] font-bold tracking-[0.12em]">
              THE PROPOSAL CHANGES THIS
            </span>
          </div>
          <div className="rounded-[12px] border p-3" style={{ borderColor: `${accent}66`, backgroundColor: `${accent}18` }}>
            ...
            <div className="mt-auto pt-3">
              <details className="group">
                <summary className="text-quiet flex cursor-pointer list-none items-center gap-[6px] font-sans text-[12px] font-semibold">
                  ...
                  {change.quote.locator ? `In the text · ${change.quote.locator}` : "In the text"}
                  <Chevron />
                </summary>
  ...
  return (
    <div className="grid gap-3 md:grid-cols-2" data-testid="brief-affected">
      {affected.map((group, index) => {
        const d = DIRECTION[group.direction] ?? (DIRECTION.unclear as (typeof DIRECTION)[string]);
        const takeaway = "takeaway" in group ? (group.takeaway as string | undefined) : undefined;
        return (
          <div
            key={index}
          ...
          >
            <div className="mb-2 flex items-center gap-2">
              <span
                ...
                style={{ backgroundColor: `${d.color}24`, borderColor: `${d.color}4A`, color: d.color }}
              >
                <Icon name={d.icon} size={14} />
              </span>
              <h3 className="flex-1 font-sans text-[14.5px] leading-[19px] font-bold">{group.group}</h3>
              <span
                className="rounded-full border px-2 py-[2px] font-sans text-[10px] font-bold tracking-[0.06em] uppercase"
                style={{ backgroundColor: `${d.color}18`, borderColor: `${d.color}70`, color: d.color }}
              >
                {d.label}
              </span>
            </div>
...
function Unknowns({ unknowns, accent }: { unknowns: string[]; accent: string }) {
  if (unknowns.length === 0) return null;
  return (
    <section className="bg-surface border-hair-2 rounded-[14px] border p-[15px] md:p-5" data-testid="brief-unknowns">
      <div className="mb-3 flex items-center gap-2">
        <Icon name="help" size={15} className="text-quiet" />
        <h2 className="font-editorial text-[16px] font-bold">What the text doesn&apos;t settle</h2>
      </div>
...
function FurtherReading({
  ...
  return (
    <div className="flex flex-col gap-3" data-testid="brief-further-reading">
      {deepDive ? (
        <details className="group bg-slate border-hair-1 rounded-[14px] border border-l-[3px] p-4" style={{ borderLeftColor: accent }}>
          <summary className="flex cursor-pointer list-none items-start gap-3">
            ...
            <span className="flex-1">
              <span className="block font-sans text-[10px] font-bold tracking-[0.12em]" style={{ color: accent }}>
                BILLION EXPLAINER
              </span>
              <span className="font-editorial mt-1 block text-[18px] leading-[23px] font-bold">{deepDive.title}</span>
              <span className="text-quiet mt-1 block font-sans text-[13.5px] leading-[20px]">
                <Emphasis text={deepDive.dek} />
              </span>
              <span className="mt-2 block font-sans text-[12.5px] font-semibold group-open:hidden" style={{ color: accent }}>
