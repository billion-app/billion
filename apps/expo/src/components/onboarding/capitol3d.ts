 ...
 * rebuilding topology.
 */
/* eslint-disable @typescript-eslint/no-non-null-assertion, @typescript-eslint/prefer-for-of */
...
function post(c: Cam, r: number, y0: number, y1: number, th: number): number[] {
  const s = Math.sin(th);
  const co = Math.cos(th);
