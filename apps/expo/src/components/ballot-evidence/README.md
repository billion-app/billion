...
These reusable components expose the limits of a lookup without deciding whether an election exists or a ballot has been published. They make no requests and do not enable Elections. Import from `./BallotEvidence` and `./model` in this directory.
...
Render `BallotSources` alongside ballot, candidate, or measure fields. Use `source` for original text, `ai-summary` for generated explanation, `citations` for a mixed-source footer whose content is labeled in the view, and `enrichment-unavailable` when the additional explanation is absent. This last state is independent of ballot status. Pass existing per-field citations without collapsing their field attribution. `MeasureCitationRef` is structurally compatible; candidate citation adapters must retain field, source name, URL and any explicit official-source evidence. Missing official flags remain unlabeled. An `ai_generated` citation tier is explicitly labeled as generated explanation. When supplied, retry must refetch the relevant read query, not start paid generation. Without a retry callback, missing source/enrichment and language states offer the official USAGov election-office directory.
...
Candidate and measure detail screens now consume the source and language components. Their route-parameter data has no verification or language evidence, so dates and language availability render as unknown. Existing AI labels remain alongside generated summaries. Missing statements and explanations describe absence from Billion, never non-submission or non-publication. Without a read query to retry, these screens link to the USAGov election-office directory. #329 owns dedicated ballot-route integration. #332 owns coverage observations; map those observations to these states only with evidence. Nationwide coverage, registration deadlines, publication status, #298 ingestion, and #299 freshness policy are not implemented here.
...
This work answers “what is on my ballot?” through contest evidence and honest limits. Arnav owns the midterms explainer and participation content; these components do not add either.
