...
import { getTabBarItemDisplay, isTabRouteHidden } from "./tab-bar-visibility";
...
function TabButton({
  ...
  const [mark] = useState(() => new Animated.Value(focused ? 1 : 0));
    ...
    >
      ...
      <View style={s.inner}>
        {state.routes.map((route, index) => {
          ...
          const iconName = TAB_ICONS[route.name] ?? "layers";
          return (
            <TabButton
              key={route.key}
              label={label}
