...
// ============================================================================
// DIGEST TOKENS — planes, hairlines, accents, radii, spacing
// Defined once in @acme/ui/digest-tokens so the web reader reads the same
// values. Re-exported here so `~/styles` stays the mobile entry point.
// ============================================================================
...
/**
 * Compatibility bag used by Digest screens. Values are aliases of `planes` +
 * `digest`, which are defined in `@acme/ui/digest-tokens`.
 */
export const DigestPalette = {
  ...
  badgeIndigo: digest.badgeIndigo,
  badgeTeal: digest.badgeTeal,
} as const;
...
export const DigestTabBar = {
  ...
  activePillBg: DigestHair.tabActivePill,
  iconSize: 22,
  labelSize: 11,
} as const;
...
/** Typography roles used by Digest chrome (import from ~/styles only). */
export const DigestFonts = {
  display: fontDisplay,
  editorial: fontEditorial,
  body: fontBody,
  /** Greeting ceremony only (iOS Snell Roundhand / GreatVibes). */
  greetingScript: {
    ios: "Snell Roundhand",
