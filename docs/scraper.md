...
4. **Dual lens** (`text-generation.ts`) — proponent/opponent arguments grounded in an agentic web-research loop, cached in `content_lens`. Bill lenses consume the same complete section-note inventory as the brief plus the official CRS summary; raw bill text never enters this path. The lens is the most expensive step and the only one whose output is not reproducible: the same input can return different arguments, and the row is overwritten in place with no history. It is therefore cached on **its own inputs** (title + grounding + article type + lens/model contract, plus the analysis-schema version for bills), not on the bill's overall `contentHash`. A routine status update no longer re-rolls the lens, while changed notes or an analysis-schema bump do.
