...
but is not its fallback. The disabled video feed does not generate or store feed
images. Optional enrichment keys below improve coverage but do not prevent the
...
| Variable                  | Requirement      | Used for                                                                                       | Default / missing behavior                                                                                              | Where to get it                                                                                                                                                                |
| ------------------------- | ---------------- | ---------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `DEMOCRACY_WORKS_API_KEY` | Launch required  | Address-scoped elections and ballots                                                           | Missing access fails closed; no Google fallback or mock ballot.                                                         | Obtain a Democracy Works API account/key with REST v2 `/elections` and `includeBallotData=true` entitlement; see [provider access](https://developers.democracy.works/api/v2). |
...
| Variable             | Requirement          | Used for                       | Default / missing behavior                                                                                           | Where to get it                                                  |
| -------------------- | -------------------- | ------------------------------ | -------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| `GROQ_API_KEY`       | Optional for Next.js | API-side civic text generation | Used first; otherwise the provider falls back to OpenRouter, OpenAI, deprecated direct DeepSeek, then skips AI.      | [Groq console](https://console.groq.com/keys).                   |
| `OPENROUTER_API_KEY` | Optional fallback    | API-side civic text generation | Used after Groq and before OpenAI.                                                                                   | [OpenRouter keys](https://openrouter.ai/settings/keys).          |
| `OPENROUTER_MODEL`   | Optional             | OpenRouter model selection     | Defaults to `deepseek/deepseek-v4-flash`; change the slug to migrate models without changing code.                   | [OpenRouter models](https://openrouter.ai/models).               |
| `OPENAI_API_KEY`     | Optional fallback    | API-side civic text generation | Used when Groq and OpenRouter are absent; it is not used by the scraper.                                             | [OpenAI API keys](https://platform.openai.com/api-keys).         |
| `DEEPSEEK_API_KEY`   | Deprecated fallback  | API-side civic text generation | Used only when Groq, OpenRouter, and OpenAI are absent; retained temporarily for credential migration compatibility. | [DeepSeek API platform](https://platform.deepseek.com/api_keys). |
...
| Variable                       | Requirement                      | Used for                                                        | Missing behavior                                                                                                                                                                                                           | Where to get it                                                                                                                                         |
| ------------------------------ | -------------------------------- | --------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `OPENROUTER_API_KEY`           | Preferred for content enrichment | Articles, summaries, image keywords, and dual-lens web research | Environment validation also accepts a local endpoint or the deprecated direct DeepSeek key for registered content scrapers.                                                                                                | [OpenRouter keys](https://openrouter.ai/settings/keys).                                                                                                 |
| `OPENROUTER_MODEL`             | Optional                         | Selects the OpenRouter text model                               | Defaults to `deepseek/deepseek-v4-flash`.                                                                                                                                                                                  | [OpenRouter models](https://openrouter.ai/models).                                                                                                      |
| `LOCAL_LLM_BASE_URL`           | Preferred local provider         | Text generation and generated-image review                      | Used before hosted text providers and before DeepSeek image review; Docker Desktop/OrbStack commonly uses `http://host.docker.internal:11434/v1` for host Ollama.                                                          | Run an OpenAI-compatible multimodal server such as Ollama.                                                                                              |
| `LOCAL_LLM_MODEL`              | Optional                         | Local text and image-review model                               | Defaults to `billion-scraper:latest`; the deployment Modelfile derives it from `gemma4:12b` with a bounded 32K context. The model must accept image URL content for review.                                                | Install or create the model in the local server.                                                                                                        |
| `LOCAL_LLM_API_KEY`            | Optional secret                  | Bearer token for protected local endpoints                      | Defaults to the ignored `ollama` placeholder.                                                                                                                                                                              | Set only if the endpoint requires authentication.                                                                                                       |
| `DEEPSEEK_API_KEY`             | Hosted fallback                  | Direct text, web-search, and generated-image review             | Text uses it after modern providers; image review uses it only when the local review is unavailable.                                                                                                                       | [DeepSeek API platform](https://platform.deepseek.com/api_keys).                                                                                        |
...
| `BFL_API_KEY`                  | Optional hosted provider         | Shared FLUX image helper; not the scheduled header-art job      | The shared helper uses BFL only after local FLUX fails. Scheduled header art calls the local-only helper and never uses this key.                                                                                          | Create a key under API → Keys in the [BFL dashboard](https://dashboard.bfl.ai); see the [BFL quick start](https://docs.bfl.ai/quick_start/get_started). |
...
| `LOCAL_FLUX_BASE_URL`          | Local image provider             | Local FLUX HTTP API                                             | Required by scheduled header art. Receives `POST /generate`; on macOS containers use `http://host.docker.internal:8080`.                                                                                                   | The Big Mac launchd template is under `deploy/big-mac`.                                                                                                 |
| `LOCAL_FLUX_MODEL`             | Optional                         | Local FLUX model name                                           | Defaults to `klein`.                                                                                                                                                                                                       | Use a model accepted by the local server.                                                                                                               |
...
Content scrapers require at least one of `OPENROUTER_API_KEY`,
`LOCAL_LLM_BASE_URL`, or deprecated `DEEPSEEK_API_KEY`. General text generation
prefers the local endpoint when configured; structured output and web research
have their own selection rules in `apps/scraper/src/utils/ai/provider.ts`.

| CLI name            | Required configuration                                     | Additional behavior                                                 |
| ------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------- |
| `whitehouse`        | `POSTGRES_URL` and a text provider                         | Presidential actions RSS and article pages                          |
| `federalregister`   | `POSTGRES_URL` and a text provider                         | Keyless Federal Register source                                     |
| `congress`          | `POSTGRES_URL`, `CONGRESS_API_KEY`, and a text provider    | Federal legislation                                                 |
| `open-states`       | `POSTGRES_URL`, `OPEN_STATES_API_KEY`, and a text provider | `OPEN_STATES_STATES` selects jurisdictions                          |
| `legistar`          | `POSTGRES_URL`                                             | Ingestion windows and document limits come from its contract        |
| `scc-cvig`          | `POSTGRES_URL`                                             | Optional `GOOGLE_GENERATIVE_AI_API_KEY` for PDF extraction fallback |
| `ca-sos-statements` | `POSTGRES_URL`                                             | Public candidate pages with PDF fallback                            |

For optional image providers and source-specific limits, use the adjacent
`*.config.ts` contract or `pnpm env:setup --target scraper --scraper NAME`.
...
- `OPENAI_API_KEY` is an API-side fallback only; the scraper uses OpenRouter
  with deprecated direct DeepSeek as its credential-migration fallback.
...
```bash
pnpm --filter @acme/scraper run start federalregister --max-items 1 --concurrency 1
```
...
pnpm --filter @acme/scraper run start open-states --max-items 1 --concurrency 1
pnpm --filter @acme/scraper run start scc-cvig --max-items 1 --concurrency 1
pnpm --filter @acme/scraper run start ca-sos-statements --max-items 1 --concurrency 1
```

For an intentional test of every registered source, including White House and Legistar, configure their contracts first. Then run the bounded concurrent suite:

```bash
pnpm --filter @acme/scraper run start all --max-items 1 --concurrency 1
```
