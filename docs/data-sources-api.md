...
**Auth:** None for the currently configured public clients; verify per jurisdiction

**Jurisdictions wired:** San Jose (`sanjose`), Santa Clara County (`santaclara`), Sunnyvale (`sunnyvaleca`)
...
const matters = await legistar.getLegislation("santaclara", {
  text: "budget",
});
const votes = await legistar.getVotes("sanjose", eventItemId);
```
