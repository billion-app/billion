...
The gated [national ballot route](../apps/expo/src/app/ballot.tsx) offers an election-office fallback until launch. When enabled, Democracy Works results from the Elections tab or [local elections](../apps/expo/src/app/local-elections.tsx) open the same ballot experience. The initial handoff reuses the caller’s query result; changing the lookup address or retrying fetches again. Lookup edits last for that mounted experience and do not replace the saved home address.
